...
Interactive 3D visualization tool for Boston Dynamics SPOT GraphNav maps
using Plotly.

## Features

- **3D Interactive Visualization**: Pan, zoom, and rotate the map in
  your browser
- **Waypoint Inspection**: Hover over waypoints to see ID, name, short
  code, position, and fiducial count
- **Delivery Location Highlighting**: Highlight specific waypoints
  (green, larger markers)
- **Point Cloud Display**: Visualize feature clouds captured at each
  waypoint
...
Use the `-a` flag to enable seed frame anchoring for accurate
positioning:
...
Highlight specific waypoints by their short codes (from `WAYPOINTS`
dict):
...
Short codes: - `al` - Aula (assembly hall) - `tv` - Triangle - `oh` -
Hauswart (caretaker's room) - `cw` - Turnhalle (gymnasium)

### Show Point Clouds

Display feature clouds captured at each waypoint (sampled to 50k points
for performance):
...
By default, only delivery locations show labels. To show all waypoint
labels:
...
Use the dropdown in the top-left to quickly toggle visibility: - All
Visible / All Hidden - Only \[Layer Name\] - Show only one layer - Hide
\[Layer Name\] - Hide specific layer
...
1.  **loader.py**: Reads GraphNav protobuf files (graph, waypoints,
    snapshots, anchors)
2.  **transformer.py**: Computes world positions using anchoring or BFS
    traversal
...
- `compute_waypoint_positions(map_data)` - Get (x, y, z) for all
  waypoints
- `compute_point_clouds(map_data)` - Extract and transform point cloud
  data
...
This viewer is an alternative to the official `graph_nav_view_map`
example from the SPOT SDK:
